...
def get_G2T():
    gloss_file_path = "/home/yejinhui/Projects/SLT/data/PHOENIX2014T/PHOENIX2014T.train.xx"
    text_file_path = "/home/yejinhui/Projects/SLT/data/PHOENIX2014T/PHOENIX2014T.train.de"

    # 打开文件并准备写入
    with open(gloss_file_path, "w", encoding="utf-8") as gloss_file, open(text_file_path, "w",
                                                                          encoding="utf-8") as text_file:
        # 遍历train_data中的所有examples
        for example in list(train_data.examples):
            # 从example中提取gloss和text
            gloss = example.gls
            text = example.txt
            gloss = " ".join(gloss)
            text = " ".join(text)
            # 将gloss和text分别写入相应的文件
